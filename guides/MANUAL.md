...
Timing-based synchronization (`Process.sleep/1`) is a common source of flaky tests. Supertester replaces this with deterministic synchronization patterns such as `cast_and_sync/4`. For best guarantees, use `Supertester.TestableGenServer` (or `strict?: true`) so missing sync handlers fail immediately.
...
`inject_table/3-4` is safest with modules implementing `__supertester_set_table__/2`
(as generated by Supertester helpers). The fallback path only works with pre-existing
environment keys and does not create dynamic atoms.
